...
This document describes the **controlled RQ3 validation suite** added
...
The suite produces five diagnostics (15 claims) totalling 18,930
...
**Documented deviation:** QFT circuits were excluded from the
structural pool. Pytket's `DefaultMappingPass` routes QFT through a
linear architecture with very high SWAP overhead (depth quadratic in
$n$); the QFT exclusion was inherited from the L1-rich predecessor
(see "Predecessor evidence" below). The instance pool is still
diverse (linear-CX entanglement vs random parametric layers) and
covers the L1 axis as intended.
...
- **L1Q within-Qiskit diagnostic**: on the 30-circuit pool with a
  ...
  split. The L1Q-C1 and L1Q-C3 Reversed verdicts are tie-driven; the
  honest reading is that Qiskit's L1 regime change is structurally
  neutral on this pool.
...
./venv/bin/python paper/experiments/scripts/run_rq3_l2_sampling_diagnostic.py
./venv/bin/python paper/experiments/scripts/run_rq3_l3_noise_diagnostic.py
./venv/bin/python paper/experiments/scripts/run_rq3_algo_diagnostic.py
./venv/bin/python paper/experiments/scripts/run_rq3_combined_diagnostic.py
./venv/bin/python paper/experiments/scripts/run_rq3_aggregate_summary.py
```

Approximate runtime (M1 Mac, single process): 14s (L1Q) + 4 min
(L2) + 84s (L3) + 7 min (ALGO) + 5 min (COMB) ≈ 18 min total.
...
- `paper/output/figures/section5/l1_within_qiskit_diagnostic_summary.csv` (3 claims)
...
- `paper/output/figures/section5/rq3_controlled_validation_summary.csv` (5 diagnostics aggregated)
...
- **RQ1 counts unchanged**: 119 papers, 457 claims, 76 specification-
  ready, 8 T1 / 0 T2 / 58 T3 / 10 T4.
...
This file records the supporting evidence for the current manuscript wording, in
which RQ3 is a controlled audit-space validation suite of five diagnostics spanning
L1 / L2 / L3 / ALGO / combined. The paper-facing cluster-bootstrap robustness summary
is the 47-record total: **24 canonical + 8 external + 15 RQ3 suite = 47/47**. This
replaces the earlier 35/35 total (canonical 24 + external 8 + the earlier four-regime
L3 diagnostic): the earlier four-regime L3 file is replaced for manuscript use by the
six-regime L3 file in this suite and is not added on top. The earlier 35/35 figure is retained only as
provenance; no reviewer-facing claim should be inferred from it.
