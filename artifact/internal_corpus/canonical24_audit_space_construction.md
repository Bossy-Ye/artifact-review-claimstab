...
| `paper/output/figures/section5/canonical24_label_map.csv` | Comparator labels (pytket>Qiskit; QAOA_p2>QAOA_p1). |
| `output/paper/icse_pack/derived/ANALYSIS/pes_margin_full.csv` | Per-claim `n_paired_cells`, `active_layers`, `share_L1`, `share_L2`, `share_L3`, `share_residual`, `dominant_variance_source`, `scores_csv`. |
...
| `output/paper/evaluation_v3/runs/W1_max2sat_second_family/scores.csv` | Blocks B + C raw cells: 1026 rows, 6 instances under both compilation_only_exact and combined_light_exact, 3 methods. *(Provenance path; not redistributed; aggregates above. The `evaluation_v3/` directory name is the original generation pipeline's internal scratch path and does not refer to any current versioning scheme.)* |
| `output/paper/evaluation_v2/runs/E1_maxcut_main/scores.csv` | Blocks D + E + F raw cells: 6930 rows, 30 instances under three presets, 3 methods. *(Provenance path; not redistributed; aggregates above. Internal scratch path of the original generation pipeline.)* |
| `claimstab/perturbations/space.py` lines 198–240 | Preset axis definitions (see §3). |
| `claimstab/tasks/maxcut.py` lines 102–135 | QAOA circuit builder showing **fixed-angle initialization** ($\gamma=0.8$, $\beta=0.4$ for the default `fixed` strategy). |

## 3. Preset definitions (from `claimstab/perturbations/space.py`)
...
## 7. Evidence gaps and redesign recommendations
...
- **ALGO axis (optimization claims, Blocks B–F).** The QAOA `objective`
  ...
  canonical optimization claim is therefore semantically a **layer-count
  comparison at fixed angles**, not an optimizer comparison. No
  experiment rerun is required for the claim as stated. If the claim
  were instead "QAOA $p=2$ outperforms $p=1$ under a real optimizer with
  stochastic initialization," ALGO would need to be added; that is a
  **redesign for a different claim**, not a fix for the current one.
  This is the most consequential disclosure in this construction note.
- **L3 axis.** No canonical block exercises L3. This is intentional;
  ...
  behavior, not a backend/noise audit. Reviewers who want to see L3
  behavior should look at the external EX-C claims (where L3 is also
  largely inactive but for source-driven reasons) and at the discussion
  of external scope in §V-C.
