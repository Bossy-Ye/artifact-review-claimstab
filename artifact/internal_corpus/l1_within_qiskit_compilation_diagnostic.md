...
This file supports the **main RQ3 L1 evidence** and records the controlled
within-Qiskit comparison used for L1Q-C1 through L1Q-C3.
...
| Axis | Status | Levels |
|---|---|---|
| INSTANCE | ACTIVE | 30 deterministic circuits (10 GHZ at $n\in\{4,5,6,7,8,9,10,11,12,14\}$; 10 hardware-efficient $n=6$ d=3 seeds 0–9; 10 hardware-efficient $n=10$ d=3 seeds 0–9) |
...
The checker derives N/k from the shipped 3,600-row grid and crosschecks all three
summary rows. The original qiskit-heavy generator is not bundled.
...
2. **Two L1 regimes only**. The diagnostic compares the extreme cases (opt=3+sabre vs opt=0+trivial); the combined-layer COMB-C1..C3 diagnostic covers additional joint L1 settings.
3. **Instance pool excludes QFT** to keep the controlled structural pool within the simulator budget.
