...
This anonymous artifact ships the **locked outputs and lightweight verification
scripts**, not the long-run simulation pipeline. Two conventions apply throughout:
...
- The long-run simulation pipeline and the per-diagnostic RQ3 raw grids
  are part of the **full research repository** and are intentionally **not bundled**
  here. The artifact instead provides the check-verified aggregates and the RQ3
  summary evidence under [internal_corpus/](internal_corpus/).
...
Dependencies for the checks are `pandas` and `pytest` (see `requirements.txt`); the
verdict-core under [src/claimstab/](src/claimstab/) is pure-Python. The optional
Tier-1 robustness regeneration additionally uses `numpy` (see [ENVIRONMENT.md](ENVIRONMENT.md)).
Each check writes a short report under `artifact/outputs/reports/` (generated on run, not tracked).
...
**Raw extraction vs. accepted count.**
[field_study/claims/extracted_claims_457.csv](field_study/claims/extracted_claims_457.csv)
holds the **raw Stage-3 extraction records before claim-level correction**: 457 records
over 118 unique papers, raw paradigm split 314 variational / 143 compilation, with
per-claim O1–O7 screen reasons. It is not the final accepted-claim file. The
paper-facing count is **455 accepted claims** (313 variational / 142 compilation after
two Layer-1 demotions), recorded in
[corrected_headline_counts.csv](corrected_headline_counts.csv) and
[source_grounded_claim_adjudication.csv](source_grounded_claim_adjudication.csv). The
...
registry-level auditable design entries. Verified by [scripts/check_headline_numbers.py](scripts/check_headline_numbers.py).
...
- [corrected_headline_counts.csv](corrected_headline_counts.csv) and [source_grounded_claim_adjudication.csv](source_grounded_claim_adjudication.csv) — corrected source-grounded RQ1 authority.
...
The per-diagnostic raw grids, the unified `rq3_controlled_validation_summary.csv`, and
the long-run simulation pipeline are part of the full research repository and are not
bundled here (see *Reproduction scope and paths*). The six-regime L3 is the
manuscript-facing L3 diagnostic; an earlier four-regime L3 diagnostic is not bundled
and is not combined or double-counted.
...
- No author names, institutions, ORCID, or contact information appear in this artifact.
- No public GitHub / GitLab / Zenodo / DOI URLs identifying the authors appear in the manuscript or in artifact-facing documents.
- All EX-C source-paper identities are anonymized through the
  [external_audit/mapping/source_mapping_anonymized.csv](external_audit/mapping/source_mapping_anonymized.csv) indirection.
