...
- **B_A:** Cirq constructor (as executed from `.external_audit_tmp/venv`).
- **B_B:** Qiskit constructor (as executed from `.external_audit_tmp/venv`).
...
- Note: the published claim cites `test_DTC100_set_build` (DTC at N=100); the audited surface narrows to constructor-only timing across `n ∈ {4,6,8,10}` because the locally executable adapter targets the constructor path, not the full Benchpress DTC100 import. This narrowing is logged in `external_claim_inventory.csv` (`local_execution_notes`).
...
- **Per-cell timing protocol (recovered).** Each `(circuit_size, repetition)` tuple is **one audit cell, with no per-cell aggregation**. The adapter `_audit_slr_cirq_vs_qiskit_construction_time` (lines 877–945 of `paper/experiments/scripts/run_external_claim_audit.py`) runs a **fresh Python subprocess per cell**, warms both constructors once, then measures a **single `time.perf_counter()` delta** around one `build_cirq()` call and one `build_qiskit()` call, and sets the cell outcome to `cirq_time < qiskit_time`. The 20 `timing_repetition` values are therefore 20 *cells along the timing axis*, not 20 measurements aggregated into one cell. This is corroborated by the locked per-cell measurements file `paper/output/audited_corpus_v2/full_cells/phase1_six_claims/details/ext_2409_08844v2_claim1_measurements.csv`, which contains 80 rows — one per `(circuit_size, repetition)` tuple — each with `cirq_time_seconds`, `qiskit_time_seconds`, and `outcome ∈ {0, 1}` columns and all 80 outcomes equal to `0` (no direction preservation), consistent with the locked $\hat{s}=0.000$.

### 8. Evidence pointers
- `per_claim_perturbations.csv` row `ext_2409_08844v2_claim1`.
- `paper/output/audited_corpus_v2/full_cells/phase1_six_claims/external_claim_audit_summary.csv` (adapter `slr_cirq_vs_qiskit_construction_time`; "host timing noise should be reported" note).
- `paper/output/audited_corpus_v2/full_cells/phase1_six_claims/details/ext_2409_08844v2_claim1_measurements.csv` (per-cell timing pairs and `outcome` values; 80 rows).
- `paper/experiments/scripts/run_external_claim_audit.py` lines 877–945 (adapter `_audit_slr_cirq_vs_qiskit_construction_time`).
- `external_claim_inventory.csv` row `ext_2409_08844v2_claim1` (paper claim text; narrowing rationale).
...
**CLEAR.** Construction-time protocol is fully recoverable: 4 × 20 = 80 fresh-subprocess single-measurement cells, outcome computed per cell, no summary statistic. The narrowing from `test_DTC100_set_build` to constructor-only timing across `n ∈ {4,6,8,10}` is recorded in `external_claim_inventory.csv` and reflected in the manuscript's Table IV row label.
...
- **P_res:** host-wallclock single-process; Pytket compiled and Qiskit compiled from the same `.external_audit_tmp/venv`.
...
- **PyZX, Cirq comparator backends:** ARTIFACT-UNSUPPORTED — the source claim names Pytket vs Qiskit/PyZX/Cirq, but PyZX is unavailable locally; the audit is narrowed to Pytket-vs-Qiskit (logged in `external_claim_inventory.csv`).
...
- **Resource budget:** FIXED-BY-SOURCE — `.external_audit_tmp/venv` invocation budget per cell.
...
- Narrowing from the broader Pytket-vs-Qiskit/PyZX/Cirq comparator set is logged in `external_claim_inventory.csv` (`local_execution_notes`).
...
- **Per-cell timing protocol (recovered).** Each `(circuit_size, qiskit_optimization_level, repetition)` triple is **one audit cell, with no per-cell aggregation**. The adapter `_audit_slr_pytket_vs_qiskit_runtime` (lines 948–1011 of `paper/experiments/scripts/run_external_claim_audit.py`) runs a fresh Python subprocess per cell, builds a deterministic QASM circuit seeded by `rep`, then measures a **single `time.perf_counter()` delta** for one `transpile(...)` call and a single delta for one Pytket `DefaultMappingPass(...) → DecomposeSwapsToCXs(...)` sequence. Cell outcome is `pytket_time < qiskit_time`. The 5 `repetition` values along this axis are 5 *cells*, not 5 measurements aggregated into one cell.
- The locked per-cell measurements file `paper/output/audited_corpus_v2/full_cells/phase1_six_claims/details/ext_2202_14025v1_legacy_claim12_measurements.csv` contains 80 rows — one per `(circuit_size, repetition, qiskit_optimization_level)` triple — with `pytket_time_seconds`, `qiskit_time_seconds`, and `outcome` columns; all 80 outcomes equal `1` (Pytket faster), consistent with the locked $\hat{s}=1.000$.
...
- `external_claim_inventory.csv` row `ext_2202_14025v1_legacy_claim12` ("Runtime metric sensitive to host environment; pairwise narrowed route").
- `external_claim_audit_summary.csv` row `ext_2202_14025v1_legacy_claim12` (adapter `slr_pytket_vs_qiskit_runtime`; Sustained).
- `paper/output/audited_corpus_v2/full_cells/phase1_six_claims/details/ext_2202_14025v1_legacy_claim12_measurements.csv` (per-cell timing pairs and `outcome` values; 80 rows).
- `paper/experiments/scripts/run_external_claim_audit.py` lines 948–1011 (adapter `_audit_slr_pytket_vs_qiskit_runtime`).
...
- **BQSKit, Staq comparator tools:** ARTIFACT-UNSUPPORTED — original claim names a 4-tool comparison; audit narrows to Qiskit-vs-Tket because the locally executable adapter pair is Qiskit and Tket (logged in `external_claim_inventory.csv`).
...
- Narrowing from the broader Qiskit vs BQSKit/Staq/Tket comparator set logged in `external_claim_inventory.csv`.
...
- `external_claim_inventory.csv` row `ext_2409_08844v2_claim3` (narrowing rationale; admissible variation candidates).
- `external_claim_audit_summary.csv` row `ext_2409_08844v2_claim3` (adapter `slr_qiskit_vs_tket_2q_gate_count`; Reversed full-cell audit).
...
- `external_claim_inventory.csv` row `ext_2304_08814v2_claim5`.
- `external_claim_audit_summary.csv` row `ext_2304_08814v2_claim5` (adapter `slr_tket_vs_qiskit_cnot_count`; Unresolved smoke-suite audit note).
- `paper/output/audited_corpus_v2/full_cells/ext_2304_08814v2_claim5.json` (per-cell log).
...
- `external_claim_inventory.csv` row `ext_2409_08844v2_claim4`.
- `external_claim_audit_summary.csv` row `ext_2409_08844v2_claim4` (adapter `slr_tket_vs_qiskit_2q_depth`; Unresolved smoke audit).
...
- `external_claim_inventory.csv` row `ext_2202_14025v1_legacy_claim6`.
- `external_claim_audit_summary.csv` row `ext_2202_14025v1_legacy_claim6` (adapter `slr_qiskit_vs_pytket_cx_depth_overhead`; Unresolved smoke audit).
...
`paper/output/audited_corpus_v2/full_cells/phase1_six_claims/details/<claim_id>_measurements.csv`
file and the adapter implementations in
`paper/experiments/scripts/run_external_claim_audit.py`.
