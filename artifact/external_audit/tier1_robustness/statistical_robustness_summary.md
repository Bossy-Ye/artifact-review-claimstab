...
C7/C8 = 4, EX-C6 = 20). The genuine limitation is **dependence-not-assessable** for
...
**6. EX-C8 contrast.** EX-C8 is the *clean* Reversed record: 0 preserved, **0 ties**,
...
**RQ4 (robustness).** Report the per-dimension status grid
(`tier1_robustness_status.csv`), not a single "robust" label. The honest headline:
...
**Threats to validity.** State plainly that S/U/R are **scoped audit-policy outcomes
over locked finite designs**, not population-level stability estimates: (i) the Wilson
