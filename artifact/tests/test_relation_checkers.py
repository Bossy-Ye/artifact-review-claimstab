...
def test_scalar_string_boolean_values_are_parsed_strictly():
    out = scalar_directional_wilson_check(
        RelationCheckInput(
            relation_type="scalar_directional",
            claim_id="t",
            params={"outcomes": ["False"] * 80},
        )
    )
    assert out.outcome.startswith("Reversed")
    assert out.quantitative_statistic["k"] == 0


@pytest.mark.parametrize("value", ["truthy", "", None, 2, object()])
def test_scalar_ambiguous_boolean_values_are_rejected(value):
    with pytest.raises(ValueError):
        scalar_directional_wilson_check(
            RelationCheckInput(
                relation_type="scalar_directional",
                claim_id="t",
                params={"outcomes": [value]},
            )
        )


@pytest.mark.parametrize("k,n", [(-1, 80), (81, 80), (0, -1)])
def test_scalar_invalid_counts_are_rejected(k, n):
    with pytest.raises(ValueError):
        _scalar(k, n)
...
def test_aggregate_nonpositive_rows_require_locked_convention():
    out = aggregate_relation_check(
        RelationCheckInput(
            relation_type="aggregate_geomean",
            claim_id="t",
            params={"ratios": [0.0, 0.5], "preservation": "ratio_lt_1"},
        )
    )
    assert out.outcome == "Boundary"
    assert "zero_failure_convention" in out.boundary_reason


def test_aggregate_unknown_preservation_condition_is_rejected():
    with pytest.raises(ValueError):
        aggregate_relation_check(
            RelationCheckInput(
                relation_type="aggregate_geomean",
                claim_id="t",
                params={"ratios": [0.5], "preservation": "unknown"},
            )
        )
...
def test_crossover_string_false_is_not_concordant():
    out = crossover_regional_check(
        RelationCheckInput(
            relation_type="aggregate_mean_curve_crossover",
            claim_id="t",
            params={"concordant": ["True", "False"]},
        )
    )
    assert out.outcome == "Discordant"
...
def test_categorical_equal_or_better_requires_explicit_complete_order():
    with pytest.raises(ValueError):
        categorical_check(
            RelationCheckInput(
                relation_type="categorical_equal_or_better",
                claim_id="t",
                params={
                    "expected_category": "equivalent",
                    "observed_category": "better",
                    "relation": "equal_or_better",
                },
            )
        )
    with pytest.raises(ValueError):
        categorical_check(
            RelationCheckInput(
                ...
                params={
                    ...
                    "relation": "equal_or_better",
                    "category_order": ["better", "equivalent", "worse"],
                },
            )
        )


def test_categorical_unknown_relation_is_rejected():
    with pytest.raises(ValueError):
        categorical_check(
            RelationCheckInput(
                relation_type="categorical_equal_or_better",
